 ...
 * @param {string} [params.ptPlanName]          - personal training plan name
 * @param {number} [params.ptPrice]             - personal training price
...
const generateBillPDF = ({
    ...
    return new Promise((resolve, reject) => {
        const hasPT = ptPrice != null && Number(ptPrice) > 0;
        const ptAmount = hasPT ? Math.round(Number(ptPrice)) : 0;
        // Taller page when PT row is present
        const doc = new PDFDocument({ size: [595, hasPT ? 600 : 540], margin: 40 });
        ...
        const membershipAmount = Math.round(offerPrice != null && offerPrice !== '' ? Number(offerPrice) : Number(price));
        ...
        const values = [
            ...
            paymentDate,
            expiryDate,
            `Rs. ${Number(membershipAmount).toLocaleString("en-IN")}`,
        ];
        ...
        // PT row (only when PT is selected)
        let extraRows = 0;
        if (hasPT) {
            extraRows = 2;
            const ptY = tableTop + rowH * 2;

            // PT row background
            doc.rect(margin, ptY, colW, rowH).fillColor("#eef4ff").fill();
            doc.rect(margin, ptY, colW, rowH).strokeColor("#c8daff").lineWidth(0.5).stroke();

            // PT row text
            doc.fillColor("#003580").font("Helvetica").fontSize(10);
            doc.text(ptPlanName || "Personal Training", cols[0] + 5, ptY + 6, { width: 95, lineBreak: false });
            doc.text("PT Add-on",                       cols[1] + 5, ptY + 6, { width: 95, lineBreak: false });
            doc.text("—",                               cols[2] + 5, ptY + 6, { width: 95, lineBreak: false });
            doc.text("—",                               cols[3] + 5, ptY + 6, { width: 95, lineBreak: false });
            doc.text(`Rs. ${ptAmount.toLocaleString("en-IN")}`, cols[4] + 5, ptY + 6, { width: 95, lineBreak: false });

            // TOTAL row
            const totY = tableTop + rowH * 3;
            doc.rect(margin, totY, colW, rowH).fillColor("#e8e8e8").fill();
            doc.rect(margin, totY, colW, rowH).strokeColor("#cccccc").lineWidth(0.5).stroke();
            doc.fillColor("#000000").font("Helvetica-Bold").fontSize(10);
            doc.text("TOTAL",                                             cols[0] + 5, totY + 6, { width: 95, lineBreak: false });
            doc.text(`Rs. ${totalAmount.toLocaleString("en-IN")}`,        cols[4] + 5, totY + 6, { width: 95, lineBreak: false });
        }

        // ── PAYMENT SUMMARY ───────────────────────────────────────────
        const summaryY = tableTop + rowH * (2 + extraRows) + 18;
