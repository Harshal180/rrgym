    ...
    try {
        ...
        await rootConn.query(`
            CREATE TABLE IF NOT EXISTS members (
                ...
                personal_training VARCHAR(50)  DEFAULT 'No',
        ...
        await rootConn.query(`
            CREATE TABLE IF NOT EXISTS payments (
                ...
                plan_name      VARCHAR(50)   NOT NULL,
                pt_plan_name   VARCHAR(100)  DEFAULT NULL,
        ...
        await addColIfMissing(
            "pt_plan_name",
            "ADD COLUMN pt_plan_name VARCHAR(100) DEFAULT NULL AFTER plan_name"
        );
        ...
        // ─── Drop legacy otp_tokens table if it exists ────────────────────────
        await rootConn.query(`DROP TABLE IF EXISTS otp_tokens`);
        console.log("✅ otp_tokens table removed (unused)");

        console.log("✅ All tables ready");

        // ─── personal_training_plans table ────────────────────────────────────
        await rootConn.query(`
            CREATE TABLE IF NOT EXISTS personal_training_plans (
                id               INT NOT NULL AUTO_INCREMENT,
        ...
        `);
        console.log("✅ personal_training_plans table ready");
        ...
        // Seed default personal training plans
        const [ptRows] = await rootConn.query("SELECT COUNT(*) AS cnt FROM personal_training_plans");
        if (ptRows[0].cnt === 0) {
            await rootConn.query(`
                INSERT INTO personal_training_plans (plan_name, duration_months, price, description) VALUES
                ('PT - 1 Month',  1,  5000.00,  '1-on-1 sessions, custom workout plan, diet consultation'),
                ('PT - 3 Months', 3,  7000.00,  '1-on-1 sessions, custom workout & nutrition plan, progress tracking'),
                ('PT - 6 Months', 6,  10000.00, '1-on-1 sessions, advanced programming, body composition analysis'),
                ('PT - 1 Year',   12, 15000.00, 'Unlimited 1-on-1 sessions, full nutrition coaching, priority support')
            `);
            console.log("✅ Default personal training plans seeded");
        }
