...
const recordPayment = async (req, res) => {
  try {
    const {
      member_id, plan_id, plan_name, pt_plan_name, total_amount, paid_amount,
    ...
    await db.query(
      `INSERT INTO payments
       (member_id, plan_id, plan_name, pt_plan_name, total_amount, paid_amount, balance_due, payment_date, payment_method, payment_type, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [member_id, plan_id || null, plan_name, pt_plan_name || null, total_amount, paid_amount, balance_due, safeDate, payment_method, payment_type, notes]
    );
  ...
  try {
    const [rows] = await db.query(
      `SELECT
          ...
          m.membership_type, m.personal_training,
