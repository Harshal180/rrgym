...
const RenewMembership = () => {
    ...
    const handleIdBlur = async () => {
        ...
        try {
            const res = await api.get(`/api/members/${form.id}`);
            const m = res.data;
    ...
    const getSelectedPtPlan = () =>
        ptPlans.find((p) => p.plan_name === form.personalTraining) || null;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await api.post(`/api/members/renew`, form);
            const data = res.data;
            ...
            const _ptPlanR = ptPlans.find((p) => p.plan_name === form.personalTraining) || null;
            const _ptPriceR = _ptPlanR ? Math.round(Number(_ptPlanR.offer_price != null && _ptPlanR.offer_price !== "" ? _ptPlanR.offer_price : _ptPlanR.price)) : 0;
            const _totalAmtR = (selectedPlan ? (applyOffer && selectedPlan.offer_price != null && selectedPlan.offer_price !== '' ? Math.round(Number(selectedPlan.offer_price)) : Math.round(Number(selectedPlan.price))) : 0) + _ptPriceR;
            ...
            billDataRef.current = {
                ...
                offerPrice: selectedPlan?.offer_price
                    ...
                    : null,
                ptPlanName: _ptPlanR?.plan_name ?? null,
                ptPrice: _ptPriceR > 0 ? _ptPriceR : null,
            ...
            try {
                await api.post(`/api/payments`, {
                    ...
                    pt_plan_name: form.personalTraining !== "No" ? form.personalTraining : null,
            ...
            setForm({ id: "", mobile: "", membershipType: "", personalTraining: "No", startDate: today });
            setFetchedMember(null);
            setPaidAmount("");
