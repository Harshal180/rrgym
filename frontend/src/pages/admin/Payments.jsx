...
export default function Payments() {
    ...
    const fetchAllPayments = async (from = dateFrom, to = dateTo) => {
        ...
        try {
            ...
            if (to) params.to = to;
            const res = await api.get(`/api/payments`, { params, withCredentials: true });
    ...
    const handleMemberIdBlur = async () => {
        ...
        try {
            const res = await api.get(`/api/payments/member/${collectForm.member_id}`);
    ...
    const handleCollect = async (e) => {
        ...
        try {
            const res = await api.post(`/api/payments/collect-balance`, collectForm);
                                    ...
                                    <tbody>
                                        {pending.map((p) => (
                                            <tr key={p.member_id}>
                                                ...
                                                <td>{p.mobile}</td>
                                                <td>{p.membership_type}{p.personal_training && p.personal_training !== "No" ? <><br /><span className="badge bg-info text-dark fw-normal mt-1" style={{ fontSize: 10 }}>+ {p.personal_training}</span></> : ""}</td>
                                                <td>
                                        ...
                                        {memberPayments.map(p => (
                                            <div key={p.id} className="d-flex justify-content-between align-items-center border-bottom py-2">
                                                <div>
                                                    ...
                                                    <span className="ms-2 small">{p.plan_name}{p.pt_plan_name ? ` + ${p.pt_plan_name}` : ""}</span>
                                    ...
                                    <tbody>
                                        {allPayments.length === 0 ? (
                                            ...
                                            <tr key={p.id}>
                                                ...
                                                <td>{p.plan_name}{p.pt_plan_name ? <><br /><span className="badge bg-info text-dark fw-normal mt-1" style={{ fontSize: 10 }}>+ {p.pt_plan_name}</span></> : ""}</td>
