...
// ─── ATTENDANCE CLEANUP — runs every day at 1:00 AM ──────────────────────────
// Deletes attendance records older than 30 days to keep the table lean
cron.schedule("0 1 * * *", async () => {
    ...
    try {
        ...
        `);
        console.log(`✅ Attendance cleanup: ${result.affectedRows} old record(s) deleted.`);
    } catch (error) {
        console.error("❌ Attendance cleanup failed:", error);
    }
});
