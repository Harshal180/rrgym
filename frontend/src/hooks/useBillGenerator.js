...
const useBillGenerator = () => {
  const generateBill = ({
    ...
    const ptAmount = ptPrice != null && ptPrice > 0 ? Math.round(Number(ptPrice)) : 0;
    ...
    const contentHeight = hasPT ? 235 : 210;
    ...
    doc.text(`Rs. ${Number(membershipAmount).toLocaleString("en-IN")}`,       col5, startY + 14);

    // PT row (only if PT selected)
    let extraRowOffset = 0;
    if (hasPT) {
      extraRowOffset = 8;
      doc.setFont("helvetica", "normal");
      doc.rect(14, startY + 16, pageWidth - 28, 8);
      doc.setFillColor(240, 248, 255);
      doc.rect(14, startY + 16, pageWidth - 28, 8, "FD");
      doc.setTextColor(0, 70, 150);
      doc.text(ptPlanName || "Personal Training",  col1, startY + 22);
      doc.text("(PT Add-on)",                      col2, startY + 22);
      doc.text("—",                                col3, startY + 22);
      doc.text("—",                                col4, startY + 22);
      doc.text(`Rs. ${Number(ptAmount).toLocaleString("en-IN")}`, col5, startY + 22);
      doc.setTextColor(0, 0, 0);

      // Total row
      doc.setFillColor(230, 230, 230);
      doc.rect(14, startY + 24, pageWidth - 28, 8, "FD");
      doc.setFont("helvetica", "bold");
      doc.text("TOTAL",                                                       col1, startY + 30);
      doc.text(`Rs. ${Number(totalAmount).toLocaleString("en-IN")}`,          col5, startY + 30);
      doc.setFont("helvetica", "normal");
      extraRowOffset = 16;
    }

    // ── PAYMENT SUMMARY ────────────────────────────────────────────
    const summaryY = startY + 30 + extraRowOffset;
