...
const AddMember = () => {
    ...
    useEffect(() => {
        ...
        api.get(`/api/personal-training-plans`).then((res) => setPtPlans(res.data));
    }, []);
    ...
    const getSelectedPtPlan = () =>
        ptPlans.find((p) => p.plan_name === member.personalTraining) || null;

    // Total = membership price + PT price (if selected)
    const getTotalPrice = () => {
        const membershipPrice = getEffectivePrice(getSelectedPlan());
        const ptPlan = getSelectedPtPlan();
        const ptPrice = ptPlan ? Math.round(Number(ptPlan.offer_price != null && ptPlan.offer_price !== "" ? ptPlan.offer_price : ptPlan.price)) : 0;
    ...
    const handleDownloadBill = () => {
        ...
        generateBill({
            ...
            ptPlanName: data.ptPlanName ?? null,
            ptPrice: data.ptPrice ?? null,
    ...
    const handleSendEmail = async () => {
        ...
        try {
            await api.post(`/api/members/${data.memberId}/send-bill`, {
                memberName: data.memberName,
                email: data.email,
                planName: data.planName,
                price: data.paidAmountOverride ?? (data.offerPrice ?? data.price),
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const res = await api.post(`/api/members/add`, formData, {
            ...
            if (isMember && newMemberId) {
                ...
                const [memberRes, plansRes] = await Promise.all([
                    api.get(`/api/members/${newMemberId}`),
                    api.get(`/api/plans`),
                ]);
                ...
                const _ptPlan2 = ptPlans.find((p) => p.plan_name === member.personalTraining) || null;
                const _ptPrice2 = _ptPlan2 ? Math.round(Number(_ptPlan2.offer_price != null && _ptPlan2.offer_price !== "" ? _ptPlan2.offer_price : _ptPlan2.price)) : 0;
                const _totalAmt2 = (matchedPlan ? (applyOffer && matchedPlan.offer_price != null && matchedPlan.offer_price !== '' ? Math.round(Number(matchedPlan.offer_price)) : Math.round(Number(matchedPlan.price))) : 0) + _ptPrice2;
                ...
                billDataRef.current = {
                    ...
                    ptPlanName: _ptPlan2?.plan_name ?? null,
                    ptPrice: _ptPrice2 > 0 ? _ptPrice2 : null,
                ...
                setMember({
                    ...
                    membershipType: "", personalTraining: "No", startDate: today, endDate: "", image: null,
                });

                // Record payment — total includes PT price if selected
                const ptPlanForPayment = ptPlans.find((p) => p.plan_name === member.personalTraining) || null;
                const ptPrice = ptPlanForPayment ? Math.round(Number(ptPlanForPayment.offer_price != null && ptPlanForPayment.offer_price !== "" ? ptPlanForPayment.offer_price : ptPlanForPayment.price)) : 0;
                const totalAmt = matchedPlan
                    ? (applyOffer && matchedPlan.offer_price != null && matchedPlan.offer_price !== '' ? Math.round(Number(matchedPlan.offer_price)) : Math.round(Number(matchedPlan.price))) + ptPrice
                    : ptPrice;
                ...
                try {
                    await api.post(`/api/payments`, {
                        ...
                        pt_plan_name: member.personalTraining !== "No" ? member.personalTraining : null,
                            ...
                            {member.memberType === "Member" && (
                                <div className="row g-3 mb-4">
                                    <div className="col-md-6">
                                        <label className="form-label fw-semibold">Personal Training</label>
                                        <select
                                            name="personalTraining"
                                            className="form-select"
                                            value={member.personalTraining}
                                            onChange={handleChange}
                                        >
                                            <option value="No">No Personal Training</option>
                                            {ptPlans.map((p) => (
                                                <option key={p.id} value={p.plan_name}>
                                                    {p.plan_name} — Rs. {Number(p.offer_price != null && p.offer_price !== "" ? p.offer_price : p.price).toLocaleString("en-IN")}
                                                </option>
                                            ))}
                                        </select>
                                        {getSelectedPtPlan() && (
                                            <div className="form-text text-muted">
                                                {getSelectedPtPlan().description || "Personal training included"}
                                            </div>
                                        )}
                                    </div>
                                    {getSelectedPtPlan() && (
                                        <div className="col-md-6">
                                            <label className="form-label">PT Price</label>
                                            <input
                                                type="text"
                                                className="form-control fw-bold text-primary"
                                                value={`Rs. ${Math.round(Number(getSelectedPtPlan().offer_price != null && getSelectedPtPlan().offer_price !== "" ? getSelectedPtPlan().offer_price : getSelectedPtPlan().price)).toLocaleString("en-IN")}`}
                                                disabled
                                            />
                                        </div>
                                    )}
                                ...
                                {member.membershipType && getSelectedPlan() && (
                                    <div className="col-md-4">
                                        <label className="form-label">
                                            Amount to Pay
                                            {getSelectedPtPlan() && <span className="badge bg-info text-dark ms-2 fw-normal" style={{ fontSize: "0.7rem" }}>Includes PT</span>}
                                        </label>
                                        <input
                                            type="text"
                                            className="form-control fw-bold text-success"
                                            value={`Rs. ${getTotalPrice().toLocaleString("en-IN")}`}
                                            disabled
                                        />
                                        {getSelectedPtPlan() && (
                                            <div className="form-text">
                                                Membership: Rs. {getEffectivePrice(getSelectedPlan()).toLocaleString("en-IN")} + PT: Rs. {Math.round(Number(getSelectedPtPlan().offer_price != null && getSelectedPtPlan().offer_price !== "" ? getSelectedPtPlan().offer_price : getSelectedPtPlan().price)).toLocaleString("en-IN")}
                                            </div>
                                        )}
                                ...
                                <div className="row g-3 mb-4">
                                    <div className="col-md-6">
                                        ...
                                        <input
                                            ...
                                            placeholder={`Full: Rs. ${getTotalPrice().toLocaleString("en-IN")}`}
                                            value={paidAmount}
                                            min={0}
                                            max={getTotalPrice()}
                                        ...
                                        {paidAmount !== "" && Math.round(Number(paidAmount)) < getTotalPrice() && (
                                            <div className="text-danger small mt-1 fw-semibold">
                                                ⚠️ Balance due: Rs. {(getTotalPrice() - Math.round(Number(paidAmount))).toLocaleString("en-IN")}
                                            </div>
                                        )}
                                    </div>
                                    <div className="col-md-6">
